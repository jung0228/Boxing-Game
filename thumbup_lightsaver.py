    ...
    if results.multi_hand_landmarks:
        for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            if player1_wrist is None:
                ...
                color = (0, 0, 255) # Red lightsaber for player 1
            else:
                player2_wrist = wrist_pos
                player2_thumb = thumb_pos
                color = (255, 0, 0) # Blue lightsaber for player 2

            # 손 랜드마크 그리기
            mp_drawing.draw_landmarks(image, hand_landmarks, mp_hands.HAND_CONNECTIONS)

            # 엄지 손가락이 펴져 있을 때 광선검 그리기
            if wrist_pos and thumb_pos:
                draw_lightsaber(image, wrist_pos, thumb_pos, color)

    # 충돌 감지 및 체력 감소
    if player1_wrist and player2_wrist:
        collision_distance = 50  # 충돌 범위 조정 가능
        distance = ((player1_wrist[0] - player2_wrist[0]) ** 2 + (player1_wrist[1] - player2_wrist[1]) ** 2) ** 0.5
